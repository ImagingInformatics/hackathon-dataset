...
try:
    result = requests.get(server['url'] + "Patient", headers={'apikey': server['apikey']},
    ...
    for file in files:
        if file.endswith(".json"):
            ...
            if resource_id is None:
                log.error("Error reading {}, make sure an ID is specified in the header comment".format(data_path))
                #raise ValueError
...
for resource_type in load_order:
    if resource_type in fhir:
        for resource in fhir[resource_type]:
            ...
            if resource_id is None:
                log.error("Error reading {}, make sure an ID is specified in the header comment".format(resource))
                raise ValueError
            ...
            if resource_id == "random":
                try:
                    ...
                    result = requests.post(url, json=resource, headers={'Content-Type': server['format'] + '+fhir', 'apikey': server['apikey']},
                                           params={'_format': server['format']})
                    result.raise_for_status()
                ...
                try:
                    ...
                    result = requests.put(url, json=resource, headers={'Content-Type': server['format'] + '+fhir','apikey': server['apikey']},
                                          params={'_format': server['format']})
                    result.raise_for_status()
                except requests.exceptions.RequestException as e:
                    raise SystemExit(e.response.text)
